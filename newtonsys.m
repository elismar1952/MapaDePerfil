function [XO FvO]=newtonsys(C,xo,tol,c0,d0,alfa)
 
  x=xo;
  MINE=100;
  R=diag([1;1;3]);%% a importacia de cada variavel h0 D Q
  
  P=eye(length(C)); %% a importancia de cada dado
  
  XO=x;
  FvO=0*C;
  last_E=-1;
  while MINE>tol

      Jv=Jfun(x,c0,d0);
      Fv=Ffun(x,c0,d0);
      
      
      E=sqrt(mean((Fv-C)'*(Fv-C)));
      
      %if (E>last_E)
      %  fprintf(stdout,'\nE:%f\r',E);
      %else
        fprintf(stdout,'   E:%f\th0:%f\tD:%f\tQ:%f\tc0:%f\td0:%f\tC:%f\r',E,x(1),x(2),x(3),mean(c0),mean(d0),mean(Fv));
      %end
      
      if(E<MINE)
        FvO=Fv;
        MINE=E;
        XO=x;
        fprintf(stdout,'\n\nMINE:%f\th0:%f\tD:%f\tQ:%f\tc0:%f\td0:%f\tC:%f\n',E,x(1),x(2),x(3),mean(c0),mean(d0),mean(Fv));
        for jj=1:length(c0)
           fprintf(stdout,'c0:%f\td0:%f\tC:%f\n',c0(jj),d0(jj),Fv(jj));
        endfor
      endif    
    
      x=x+((Jv')*P*Jv+alfa*R)^(-1)*Jv'*P*(C-Fv);

      if sum(Fv<0)>0
        x=abs(x+0.1*rand(size(x)).*x);
      end
      
      h0=x(1);
      D=x(2);
      Q=x(3);
      
      %% h0 esta em pixel
      if h0<0
         h0=-h0;
      endif
      if h0>800
         h0=rand(1)*800;%% Se ultrapassar 100 identificar um ponto aleatorio entre 0 e 100
      endif
      
      if D<0
         D=-D;
      endif
      if D>100
         D=rand(1)*100;%% Se ultrapassar 100 identificar um ponto aleatorio entre 0 e 100
      endif
      
      if Q>pi/2 
        Q=rand(1)*pi/2 ;
      endif
      if Q<0
         Q=-Q;
      endif
      
      x(1)=h0;
      x(2)=D;
      x(3)=Q;
      
      last_E=E;
      
  endwhile
  
endfunction